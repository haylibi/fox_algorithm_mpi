...
start_time=$(date +"%s")
echo "mpirun -n 4 \"$MPI_EXECUTABLE\" < \"$INPUT_DIR/input600\" > \"$OUTPUT_DIR/output600.txt\" 2>&1"
mpirun -n 4 "$MPI_EXECUTABLE" < "$INPUT_DIR/input600" > "$OUTPUT_DIR/output600.txt" 2>&1
echo "Elapsed Time: $(($(date +"%s") - start_time)) seconds"

start_time=$(date +"%s")
echo "mpirun -n 4 \"$MPI_EXECUTABLE\" < \"$INPUT_DIR/input900\" > \"$OUTPUT_DIR/output900.txt\" 2>&1"
mpirun -n 4 "$MPI_EXECUTABLE" < "$INPUT_DIR/input900" > "$OUTPUT_DIR/output900.txt" 2>&1
echo "Elapsed Time: $(($(date +"%s") - start_time)) seconds"

start_time=$(date +"%s")
echo "mpirun -n 4 \"$MPI_EXECUTABLE\" < \"$INPUT_DIR/input1200\" > \"$OUTPUT_DIR/output900.txt\" 2>&1"
mpirun -n 4 "$MPI_EXECUTABLE" < "$INPUT_DIR/input1200" > "$OUTPUT_DIR/output900.txt" 2>&1
echo "Elapsed Time: $(($(date +"%s") - start_time)) seconds"
